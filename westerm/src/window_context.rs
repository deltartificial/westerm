...
use westerm_terminal::event::Event as TerminalEvent;
use westerm_terminal::event_loop::{EventLoop as PtyEventLoop, Msg, Notifier};
use westerm_terminal::grid::{Dimensions, Scroll};
use westerm_terminal::index::Direction;
use westerm_terminal::sync::FairMutex;
use westerm_terminal::term::test::TermSize;
use westerm_terminal::term::{Term, TermMode};
use westerm_terminal::tty;
...
/// Event context for one individual Westerm window.
...
impl WindowContext {
    ...
    fn new(
        ...
        // Create context for the Westerm window.
        Ok(WindowContext {
            preserve_title,
            terminal,
            display,
    ...
    pub fn draw(&mut self, scheduler: &mut Scheduler) {
        ...
        if !self.display.visual_bell.completed() {
            // We can get an OS redraw which bypasses westerm's frame throttling, thus
