...
pub fn spawn_ipc_socket(
    ...
    if options.daemon {
        println!("WESTERM_SOCKET={}; export WESTERM_SOCKET", socket_path.display());
    }
...
/// Send a message to the active Westerm socket.
...
fn find_socket(socket_path: Option<PathBuf>) -> IoResult<UnixStream> {
    ...
    for entry in fs::read_dir(socket_dir())?.filter_map(|entry| entry.ok()) {
        ...
        // Skip files that aren't Westerm sockets.
...
fn socket_prefix() -> String {
    ...
    format!("Westerm-{}", display.replace('/', "-"))
}
