...
use westerm_terminal::event::{EventListener, OnResize, WindowSize};
use westerm_terminal::grid::Dimensions as TermDimensions;
use westerm_terminal::index::{Column, Direction, Line, Point};
use westerm_terminal::selection::Selection;
use westerm_terminal::term::cell::Flags;
use westerm_terminal::term::{
    self, LineDamageBounds, MIN_COLUMNS, MIN_SCREEN_LINES, Term, TermDamage, TermMode,
};
use westerm_terminal::vte::ansi::{CursorShape, NamedColor};
...
impl Display {
    pub fn new(
        ...
        // Always focus new windows, even if no Westerm window is currently focused.
        #[cfg(target_os = "macos")]
        window.focus_window();
    ...
    pub fn draw<T: EventListener>(
        ...
        // Add damage from westerm's UI elements overlapping terminal.
        ...
        if matches!(self.raw_window_handle, RawWindowHandle::Xcb(_) | RawWindowHandle::Xlib(_)) {
            ...
            // will block to synchronize (this is `glClear` in Westerm), which causes a
