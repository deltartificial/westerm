...
use westerm_config::SerdeReplace;
use clap::{ArgAction, Args, Parser, Subcommand, ValueHint};
use log::{LevelFilter, error};
use serde::{Deserialize, Serialize};
use toml::Value;

use westerm_terminal::tty::Options as PtyOptions;
...
/// CLI options for the main Westerm executable.
...
pub struct Options {
    ...
    /// X11 window ID to embed Westerm within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
    pub embed: Option<String>,

    /// Specify alternative configuration file [default:
    /// $XDG_CONFIG_HOME/westerm/westerm.toml].
    #[cfg(not(any(target_os = "macos", windows)))]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: %APPDATA%\westerm\westerm.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: $HOME/.config/westerm/westerm.toml].
    #[cfg(target_os = "macos")]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
pub struct WindowIdentity {
    /// Defines the window title [default: Westerm].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: Westerm].
...
/// Send a message to the Westerm socket.
#[cfg(unix)]
#[derive(Args, Debug)]
...
pub enum SocketMessage {
    /// Create a new window in the same Westerm process.
    CreateWindow(WindowOptions),

    /// Update the Westerm configuration.
    Config(IpcConfig),

    /// Read runtime Westerm configuration.
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "westerm.bash"),
            (Shell::Fish, "westerm.fish"),
            (Shell::Zsh, "_westerm"),
        ] {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "westerm", &mut generated);
        ...
        //
        // let mut file = File::create("../extra/completions/westerm.bash").unwrap();
        // clap_complete::generate(Shell::Bash, &mut clap, "westerm", &mut file);
        // let mut file = File::create("../extra/completions/westerm.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "westerm", &mut file);
        // let mut file = File::create("../extra/completions/_westerm").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "westerm", &mut file);
    }
}
