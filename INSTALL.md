...
If you're just interested in the Westerm binary and you don't need the
...
Before compiling Westerm, you'll have to first clone the source code:

```sh
git clone https://github.com/westerm/westerm.git
cd westerm
```
...
These are the minimum dependencies required to build Westerm, please note
...
to build Westerm. Here's an apt command that should install all of them. If
...
On Arch Linux, you need a few extra libraries to build Westerm. Here's a
...
On Fedora, you need a few extra libraries to build Westerm. Here's a `dnf`
...
On CentOS/RHEL 7, you need a few extra libraries to build Westerm. Here's a `yum`
command that should install all of them. If something is still found to be
missing, please open an issue.
...
On RHEL 8, like RHEL 7, you need a few extra libraries to build Westerm. Here's a `dnf`
command that should install all of them. If something is still found to be
missing, please open an issue.
...
On openSUSE, you need a few extra libraries to build Westerm. Here's
...
On FreeBSD, you need a few extra libraries to build Westerm. Here's a `pkg`
command that should install all of them. If something is still found to be
missing, please open an issue.
...
Westerm, plus Python 3 to build its XCB dependency. If something is still
...
The default user limits in OpenBSD are insufficient to build Westerm. A
...
Westerm. Here's a `eopkg` command that should install all of them. If
something is still found to be missing, please open an issue.
...
```sh
nix-shell -A westerm '<nixpkgs>'
```

#### Gentoo

On Gentoo, you need a few extra libraries to build Westerm. The following
command should install all of them. If something is still found to be missing,
please open an issue.

```sh
emerge --onlydeps x11-terms/westerm
```

#### Clear Linux

On Clear Linux, you need a few extra libraries to build Westerm. Here's a
...
```sh
guix environment westerm
```

#### Alpine Linux

On Alpine Linux, you need a few extra libraries to build Westerm. Here's an
...
If you build Westerm on another distribution, we would love some help
...
On Linux/BSD, if it is desired to build Westerm without support for either the
...
If all goes well, this should place a binary at `target/release/westerm`.
...
There are some extra things you might want to set up after installing Westerm.
All the post build instruction assume you're still inside the Westerm
repository.

### Terminfo

To make sure Westerm works correctly, either the `westerm` or
`westerm-direct` terminfo must be used. The `westerm` terminfo will be
picked up automatically if it is installed.

If the following command returns without any errors, the `westerm` terminfo is
already installed:
...
```
sudo tic -xe westerm,westerm-direct extra/westerm.info
```

### Desktop Entry

Many Linux and BSD distributions support desktop entries for adding applications
to system menus. This will install the desktop entry for Westerm:

```sh
sudo cp target/release/westerm /usr/local/bin # or anywhere else in $PATH
sudo cp extra/logo/westerm-term.svg /usr/share/pixmaps/Westerm.svg
sudo desktop-file-install extra/linux/Westerm.desktop
sudo update-desktop-database
```

If you are having problems with Westerm's logo, you can replace it with
...
scdoc < extra/man/westerm.1.scd | gzip -c | sudo tee /usr/local/share/man/man1/westerm.1.gz > /dev/null
scdoc < extra/man/westerm-msg.1.scd | gzip -c | sudo tee /usr/local/share/man/man1/westerm-msg.1.gz > /dev/null
scdoc < extra/man/westerm.5.scd | gzip -c | sudo tee /usr/local/share/man/man5/westerm.5.gz > /dev/null
scdoc < extra/man/westerm-bindings.5.scd | gzip -c | sudo tee /usr/local/share/man/man5/westerm-bindings.5.gz > /dev/null
```

### Shell completions

To get automatic completions for Westerm's flags and arguments you can install the provided shell completions.

#### Zsh

To install the completions for zsh, you can place the `extra/completions/_westerm` file in any
...
```sh
cp extra/completions/_westerm ${ZDOTDIR:-~}/.zsh_functions/_westerm
```

#### Bash

To install the completions for bash, you can `source` the `extra/completions/westerm.bash` file
in your `~/.bashrc` file.

If you do not plan to delete the source folder of westerm, you can run

```sh
echo "source $(pwd)/extra/completions/westerm.bash" >> ~/.bashrc
```
...
cp extra/completions/westerm.bash ~/.bash_completion/westerm
echo "source ~/.bash_completion/westerm" >> ~/.bashrc
```
...
cp extra/completions/westerm.fish $fish_complete_path[1]/westerm.fish
```
