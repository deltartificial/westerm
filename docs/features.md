...
This document gives an overview over Westerm's features beyond its terminal
emulation capabilities. To get a list with supported control sequences take a
look at [Westerm's escape sequence support](./escape_support.md).

## Vi Mode

The vi mode allows moving around Westerm's viewport and scrollback using the
...
Search allows you to find anything in Westerm's scrollback buffer. You can
...
either feeds them to an external application or triggers one of Westerm's
built-in actions.
...
Westerm supports running multiple terminal emulators from the same Westerm
instance. New windows can be created either by using the `CreateNewWindow`
keybinding action, or by executing the `westerm msg create-window` subcommand.
